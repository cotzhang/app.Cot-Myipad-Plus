...
window.onload = function() {
	...
	webview.addEventListener('ipc-message', (event) => {
		if (event.channel == "logindial") {
			...
			currDiagId = panelistic.dialog.confirm(event.args[0], event.args[1], "复制用户GUID", event.args[2], (copyg) => {
				if (copyg) {
					const newInput = document.createElement('input');
					document.body.appendChild(newInput)
					newInput.value = event.args[3];
					newInput.select();
...
ClientPath: /data/user/0/data/app/com.netspace.myipad/base.apk
ClientMD5: 629f2237332f192d3a072d0da6e801b7
AppKey: MyiPad
